...
#pragma warning restore 414
    ...
    public partial class ObjectTranslator
	{
        ...
        Dictionary<int, string> deletage_tracebacks = new Dictionary<int, string>();
        public object CreateDelegateBridge(RealStatePtr L, Type delegateType, int idx)
        {
            ...
            {
                //UnityEngine.Debug.Log("CreateDelegateBridge:" + reference);
                deletage_tracebacks[reference] = delegateType.ToString();
            }
        ...
        public bool AllDelegateBridgeReleased()
        {
            ...
            foreach (var kv in delegate_bridges)
            {
                if (kv.Value.IsAlive)
                {
                    ...
                    if (deletage_tracebacks.TryGetValue(kv.Key, out string traceback))
                    {
                        UnityEngine.Debug.LogError($"Delegate Not Released:{kv.Key}\n{traceback}");
                    }
                    else
                    {
                        UnityEngine.Debug.Log($"Delegate Not Released {kv.Key}");
                    }
                }
            }
