...
namespace MikuLuaProfiler
{
...
#endif
#if UNITY_EDITOR_WIN
            if(!LuaDeepProfilerSetting.Instance.isOpenLuaProfiler)
            {
                UnityEngine.Debug.Log("<color=#ff8400>catch : luaProfiler调试连接没有打开，无需调试就不用管他</color>:");
                return;
            }
            UnityEngine.Debug.Log("<color=#ff8400>catch : 编辑器下luaProfiler调试已被打开</color>:");
            string path = null;
    ...
    public class LuaHook
    {
        public static byte[] Hookloadbuffer(IntPtr L, byte[] buff, string name)
        {
            ...
            if (LuaDeepProfilerSetting.Instance.luaDir.Count == 1)
            {
                fileName = fileName.Replace(LuaDeepProfilerSetting.Instance.luaDir[0], LuaDeepProfilerSetting.Instance.luaDirName);
            }
            // utf-8
