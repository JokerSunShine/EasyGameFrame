...
namespace MikuLuaProfiler
{
    ...
    public class LuaProfilerWindow : EditorWindow
    {
        ...
        private static double lastAutoSaveColumsTime = 0;
        #endregion
        ...
        void OnGUI()
        {
            ...
            //{
            //    var visibleColumns = m_TreeView.multiColumnHeader.state.visibleColumns;
            //    for(int i=0;i< visibleColumns.Length;i++)
            //    {
            //        Debug.Log(visibleColumns[i]);
            //    }

            //    var columns = m_TreeView.multiColumnHeader.state.columns;
            //    for(int i=0;i<columns.Length;i++)
            //    {
            //        var col = columns[i];
            //        Debug.Log(col.contextMenuText);
            //    }
            //}
            AutoSaveColums();
        }
        ...
        void AutoSaveColums()
        {
            if (EditorApplication.timeSinceStartup - lastAutoSaveColumsTime < 1) return;
            lastAutoSaveColumsTime = EditorApplication.timeSinceStartup;
            if(m_TreeView.multiColumnHeader.state.visibleColumns.Length!= LuaProfilerPrecompileSetting.Instance.visibleColumns.Count)
            {
                var visibleColumns = m_TreeView.multiColumnHeader.state.visibleColumns;
                LuaProfilerPrecompileSetting.Instance.visibleColumns.Clear();
                LuaProfilerPrecompileSetting.Instance.visibleColumns.AddRange(visibleColumns);
                EditorUtility.SetDirty(LuaProfilerPrecompileSetting.Instance);
            }
        }
        ...
        void DoToolbar()
        {
            ...
            flag = GUILayout.Toggle(setting.isOpenLuaProfiler, "Open LuaProfiler", EditorStyles.toolbarButton);
            if (flag != setting.isOpenLuaProfiler)
            {
                setting.isOpenLuaProfiler = flag;
            }
