...
#endif
    public enum DataType { Screenshot, PSS, Device, Customized, OverdrawScreenshot, GPUMali, CustomizedAction, EndLabel };
    public static class NetworkServer
    {
        ...
        public static void Close()
        {
            try
            {
                ...
                int waitCnt = 0;
                while (m_sampleQueue.Count > 0 && waitCnt < 3)
                {
                    Debug.LogError("wait " + m_sampleQueue.Count);
                    Thread.Sleep(1000);
                    waitCnt++;
                }
                lock (m_sampleQueue)
        ...
        private static void DoSendMessage()
        {
            ...
            while (keepsend)
            {
                try
                {
                    ...
                    {
                        while (m_sampleQueue.Count > 0 && ns.CanWrite)
                        {
                            UPRMessage s = null;
                            lock (m_sampleQueue)
                            {
        ...
        private static bool DealReceivedMessage(string msg)
        {
            ...
            {
                isConnected = true;
                UPROpen.onStartPackageProfiling();
                sendDeviceInfo = false;
            }
            else if (msg.Contains("Stop Sending Message") && isConnected)
            {
                UPROpen.onStopPackageProfiling();
                SendMessage(new UPRMessage{ type = (int)DataType.EndLabel});
                Close();
                return false;
            }
