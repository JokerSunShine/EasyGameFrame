...
namespace UPRLuaProfiler
{
    ...
    public static unsafe class HookUtils
    {
        ...
        {
            Func<IntPtr, string> formatAddr =
 (IntPtr addr__) => IntPtr.Size == 4 ? $"0x{(uint)addr__:x}" : $"0x{(ulong)addr__:x}";
        ...
        static HookUtils()
        {
            ...
            if(IntPtr.Size == 4)
            {
                ...
                s_ptr_flush_icache_arm32 =
 GCHandle.Alloc(s_flush_icache_arm32, GCHandleType.Pinned).AddrOfPinnedObject().ToPointer();
 ...
 Marshal.GetDelegateForFunctionPointer<DelegateFlushICache>(new IntPtr(s_ptr_flush_icache_arm32));
            ...
            {
                s_ptr_flush_icache_arm64 =
 GCHandle.Alloc(s_flush_icache_arm64, GCHandleType.Pinned).AddrOfPinnedObject().ToPointer();
