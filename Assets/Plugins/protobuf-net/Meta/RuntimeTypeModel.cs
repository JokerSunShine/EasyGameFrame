...
namespace ProtoBuf.Meta
{
    ...
    public sealed class RuntimeTypeModel : TypeModel
    {
        ...
        /// <summary>
        /// Global default that
        /// enables/disables automatic tag generation based on the existing name / oder
...
//#endif
        ...
        /// [DataContract]/[DataMember(oder=n)]
        /// [XmlType]/[XmlElement(oder=n)]
        ...
        internal void ReleaseLock(int opaqueToken)
        {
            if (opaqueToken != 0)
            {
                ...
                if(opaqueToken != GetContention()) // contention-count changes since we looked!
                {
                    ...
                    if (handler != null)
                    {
                        // not hugely elegant, but this is such a far-corner-case that it doesn't need to be slick - I'll settle for cross-plator
                        string stackTrace;
                        try
                        {
                            throw new Exception();
