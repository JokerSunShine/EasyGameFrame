...
namespace CustomDataStruct
{
    ...
    {
        private static Queue<ValueObject> mValueObjPool = new Queue<ValueObject>(32);
        private object mTypeObj;
        private ObjPoolBase mTypeObjHolder;
        private Type mObjType;

#if UNITY_EDITOR
        private static MemoryLeakDetecter detecter = MemoryLeakDetecter.Add(typeof(ValueObject).FullName, 1000, 1000);
#endif
        ...
        /// <returns></returns>
        public static object TryGet(object valueObj)
        {
        ...
        /// <returns></returns>
        public static ValueObject Get(object valueObj)
        {
#if UNITY_EDITOR
            if (!valueObj.GetType().IsValueType)
            {
            ...
            {
                return Get((TimeSpan)valueObj);
            }

            if (type == typeof(Guid))
            {
                return Get((Guid)valueObj);
            }
        ...
        /// <returns></returns>
        public static ValueObject Get<T>(T value)
        {
#if UNITY_EDITOR
