...
namespace CustomDataStruct
{
    ...
    {
#if UNITY_EDITOR
        private MemoryLeakDetecter localDetecter = MemoryLeakDetecter.Add(typeof(string).FullName, 1000, 1000);
        private static MemoryLeakDetecter globalDetecter = MemoryLeakDetecter.Add(typeof(string).FullName, 1000, 1000);
#endif
        ...
        private BetterStringBuilderBufferType mBufferType;
        private static List<string> mGlobalBuffer;
        private List<string> mLocalBuffer;
        ...
        public void Append(double value, int precision)
        {
            ...
            if (fractionalPartEx < 0)
            {
                fractionalPartEx = -fractionalPartEx;
            }
        ...
        public bool PartEquals(string str, int selfPartStartIndex, int slefPartLength)
        {
            ...
            {
                if (selfPartStartIndex < 0 || selfPartStartIndex >= Length)
                {
                    return false;
                }

                if (slefPartLength < 0 || slefPartLength > Length - selfPartStartIndex)
                {
                    return false;
                }

                if (slefPartLength != str.Length)
                {
                    return false;
                }
