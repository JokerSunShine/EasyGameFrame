...
namespace ProtoBuf
{
    ...
    public abstract class Extensible : IExtensible
    {
        ...
        public static bool TryGetValue<TValue>(IExtensible instance, int tag, DataFormat format, bool allowDefinedTag, out TValue value)
        {
            ...
            bool set = false;
            IEnumerator<TValue> item = ExtensibleUtil.GetExtendedValues<TValue>(instance, tag, format, true, allowDefinedTag).GetEnumerator();
            while (item.MoveNext())
            {
        ...
        public static bool TryGetValue(TypeModel model, System.Type type, IExtensible instance, int tag, DataFormat format, bool allowDefinedTag, out object value)
        {
            ...
            IEnumerator item = ExtensibleUtil.GetExtendedValues(model, type, instance, tag, format, true, allowDefinedTag).GetEnumerator();
            while (item.MoveNext())
            {
                value = item.Current;
                set = true;
